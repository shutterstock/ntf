      ...
      trace: function(expr, val, path) {
         if (expr) {
            ...
            if (val && val.hasOwnProperty(loc))
               ...
               P.trace(x, val, path);
               P.walk(loc, x, val, path, function(m,l,x,v,p) { if (typeof v[m] === "object") P.trace("..;"+x,v[m],p+";"+m); });
            }
               ...
               P.trace(P.run(loc, val, path.substr(path.lastIndexOf(";")+1))+";"+x, val, path);
            else if (/^\?\(.*?\)$/.test(loc)) // [?(expr)]
               P.walk(loc, x, val, path, function(m,l,x,v,p) { if (P.run(l.replace(/^\?\((.*?)\)$/,"$1"),v[m],m)) P.trace(m+";"+x,v,p); });
