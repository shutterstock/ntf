...
var subTestType = function(test, type) {
  return function() {
    ...
    if (!args[0].hasOwnProperty('jar') && test._ntf.parentOpts.hasOwnProperty('jar')) {
      args[0].jar = test._ntf.parentOpts.jar
    }
    args.unshift(test)
...
TEST.request = function(test, opts, callback) {
  ...
  if (opts.jar && typeof(opts.jar) !== 'object') opts.jar = {}
