...
var exports = module.exports = function(url) {
  // url can be a function
  if (typeof(url) === 'function') url = url()

  return {
    del: testType(url, 'del'),
    get: testType(url, 'get'),
    post: testType(url, 'post'),
    put: testType(url, 'put'),
  }
}
...
var testType = function(url, type) {
  return function() {
    var baseArgs = Array.prototype.slice.call(arguments)

    // url can be a function
    if (typeof(baseArgs[0]) === 'function') baseArgs[0] = baseArgs[0]()

    var name = [url + baseArgs[0], type].join(utils.SEP)
